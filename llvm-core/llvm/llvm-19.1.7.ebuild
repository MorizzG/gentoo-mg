...
multilib_src_install() {
	...
	# MODIFICATION: Manually install static libraries if requested
	if use static-libs; then
		# The build directory structure for static libs is usually just inside /lib
		# We look inside the cmake build directory for .a files starting with libLLVM

		# Note: We use dolib.a to install them to the correct /usr/lib/llvm/.../lib location
		# The 'into' directory is already set by the eclass context or defaults,
		# but we want them specifically in LLVM's libdir.

		local libdir=$(get_libdir)

		insinto /usr/lib/llvm/${LLVM_MAJOR}/${libdir}

		# Loop ensures we only get what we built, handling the 'lib' prefix correctly
		local lib
		for lib in "${BUILD_DIR}/${libdir}"/libLLVM*.a; do
			[[ -f ${lib} ]] && doins "${lib}"
		done
	fi
